<template>
  <div>
    ...
    <div class="filters">
      <div>
        <button @click="applyFilters({type:'heads'})">Filter for Heads</button>
      </div>
      <div>
        <button @click="applyFilters({type:'arms'})">Filter for Arms</button>
      </div>
      <div>
        <button @click="applyFilters({type:'torsos'})">Filter for Torsos</button>
      </div>
      <div>
        <button @click="applyFilters({type:'bases'})">Filter for Bases</button>
      </div>
      <div>
  ...
  <div>
    <ul>
      <li v-for="(result, index) in pagedResults" :key="index">
        <div><img :src="result.src" alt=""/></div>
        <div>
